...
printf 'RAW ARGS (%d): ' "$#" >> /var/log/vmkfstools-wrapper.log
for a in "$@"; do printf '[%s] ' "$a" >> /var/log/vmkfstools-wrapper.log; done
printf '\n' >> /var/log/vmkfstools-wrapper.log
echo "DEBUG: source_vmdk=$source_vmdk" >> /var/log/vmkfstools-wrapper.log
echo "DEBUG: target_lun=$target_lun" >> /var/log/vmkfstools-wrapper.log
echo "DEBUG: verify_md5_value=$verify_md5_value" >> /var/log/vmkfstools-wrapper.log
...
echo "DEBUG: source_vmdk_flat=$source_vmdk_flat" >> /var/log/vmkfstools-wrapper.log
...
if [ "$do_md5_check" = "true" ]; then
    ...
    if [ "$hash_mismatch" = false ]; then
        ...
        echo "DEBUG: Source HEAD MD5: $HEAD_MD5_SRC" >> /var/log/vmkfstools-wrapper.log
        echo "DEBUG: Source TAIL MD5: $TAIL_MD5_SRC" >> /var/log/vmkfstools-wrapper.log
        echo "DEBUG: Target HEAD MD5: $HEAD_MD5_TGT" >> /var/log/vmkfstools-wrapper.log
        echo "DEBUG: Target TAIL MD5: $TAIL_MD5_TGT" >> /var/log/vmkfstools-wrapper.log
