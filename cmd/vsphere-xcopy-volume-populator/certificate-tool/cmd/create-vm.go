...
func createVM(ctx context.Context, client *govmomi.Client, cfg VMConfig, datastore, vmName string) (*object.VirtualMachine, error) {
	...
	// finder expects the *Datacenter, not its reference
	finder.SetDatacenter(dc)
...
func attachCDROM(ctx context.Context, vm *object.VirtualMachine, isoPath string) error {
	// Retrieve current devices on the VM
	devices, err := vm.Device(ctx)
	if err != nil {
		return fmt.Errorf("list devices: %w", err)
	}

	// Pick an IDE or SATA controller (first found)
	...
	// Create a blank CD‑ROM attached to that controller
	cdrom, err := devices.CreateCdrom(controller)
	if err != nil {
		return fmt.Errorf("create cdrom device: %w", err)
	}

	// Point the CD‑ROM backing to the ISO
...
var createVmCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		// ------------------------------------------------------------------
		// 7. Wait until the VM appears in inventory (race‑free)
		// ------------------------------------------------------------------
		...
		// ------------------------------------------------------------------
		// 8. Inside‑guest: write random data for later copy tests
		// ------------------------------------------------------------------
