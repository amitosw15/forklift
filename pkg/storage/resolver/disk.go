...
func DiskBackingFromDevices(devices []types.BaseVirtualDevice, diskFile string) (*DiskBacking, error) {
	...
	for _, device := range devices {
		...
		case *types.VirtualDiskFlatVer2BackingInfo:
			if !matchesDiskFile(backing.FileName, normalizedPath, diskFile) {
				continue
			}
...
func matchesDiskFile(fileName, normalizedPath, diskFile string) bool {
	if strings.Contains(strings.ToLower(fileName), normalizedPath) ||
		strings.Contains(normalizedPath, strings.ToLower(fileName)) {
		return true
	}
	return diskPathMatches(fileName, diskFile)
}
